...
def index():
    ...
    for i in range(len(stockRows)):
        if not stockRows[i]["shares"] == 0:
            symbol = stockRows[i]['symbol']
            result = lookup(symbol)
            if result is None:
                apoltext = f"lookup error for {stockRows[i]['symbol']}: {result}"
                return apology(apoltext, 403)
            currPrice = result["price"]
            positionValue = currPrice * stockRows[i]["shares"]
            positionValue = int(positionValue)
            stocks.append((stockRows[i]["symbol"], usd(round(currPrice, 2)), stockRows[i]["shares"], usd(round(positionValue, 2))))
...
def buy():
    ...
    if request.method == "POST":
        ...
        if (not (shares > 0)) or (not (shares % 1 == 0)):
            return apology("shares must be a positive integer", 400)

        if result is None:
            apoltext = f"lookup error for {symbol}: {result}"
            return apology(apoltext, 400)
        ...
        if totalPrice > cashAvailable:
            return apology("sorry, you don't have enough money to buy that", 400)
        ...
        if not stockRows:
            ...
            apoltext = f"database error: {stockRows}"
            return apology(apoltext, 400)
...
def quote():
    ...
    if request.method == "POST":
        ...
        if result is None:
            apoltext = f"lookup error for {symbol}"
            return apology(apoltext, 400)
...
def register():
    ...
    if request.method == "POST":
        ...
        if len(rows) != 0:
            return apology("sorry, that username exists already", 400)
        ...
        db.execute("INSERT INTO users (username, hash) VALUES (:username,:hashedpass)",
                    username=request.form.get("username"), hashedpass=hashedpass)
...
def sell():
    ...
    if request.method == "POST":
        ...
        if not stocksRows[0]["symbol"] == symbol:
            return apology("sorry, you don't own any shares of that stock.", 400)

        # check whether they have enough shares of that stock
        if stocksRows[0]["shares"] < shares:
            return apology("sorry, you don't own that many shares of this stock", 400)
        ...
        newCash = cashRows[0]["cash"] + (sellPrice * shares)
        db.execute("UPDATE users SET cash=:newCash WHERE id=:userId", newCash = round(newCash, 2), userId = userId)
    ...
    if request.method == "GET":
        userId = session.get("user_id")
        stocks = db.execute("SELECT * FROM stocks WHERE id = :userId AND shares > 0", userId = userId)
